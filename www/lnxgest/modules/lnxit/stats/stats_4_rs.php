...
echo '<h2>Estadisticas tickets por categorias</h2>';
...
<?php
$ConsultaMySql= $mysqli->query("SELECT id, categoria from ".$prefixsql."it_categorias order by categoria");

while($columna = mysqli_fetch_array($ConsultaMySql))
{
	
	$ssql = "SELECT * FROM ".$prefixsql."it_tickets where fcreacion >= '".$cfechainicio."' and fcreacion <= '".$cfechafin."' and idcategoria = '".$columna["id"]."' ";
	...
	if ($nregistros > '0')
	{
		echo '<tr class="maintitle"><td colspan="8">Categoria: '.$columna["categoria"].'</td></tr>';
		echo '<tr class="maintitle"><td>Ticket</td><td>Estado</td><td>Fecha</td><td>resumen</td><td>Asignado</td><td>tiempo</td><td>Computado</td><td>Facturado</td></tr>';
		$color = '1';
		while($coltickets = mysqli_fetch_array($cnssql))
		{
			...
			echo '<td>'.$estado.'</td>';
			echo '<td>'.$coltickets["fcreacion"].'</td>';
			echo '<td>'.$coltickets["resumen"].'</td>';
			
			$cnsauxtempo= $mysqli->query("SELECT id, display from ".$prefixsql."users where id = '".$coltickets["idasignado"]."' ");
			$row = mysqli_fetch_assoc($cnsauxtempo);

			$Asignado = $row["display"];
