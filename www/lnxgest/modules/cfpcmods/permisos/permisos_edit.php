...
$sqltipobanco= $mysqli->query("select * from ".$prefixsql."permisos where id = '".$fidpermiso."'");
$row = mysqli_fetch_assoc($sqltipobanco);
$dbidmod = $row["idmod"];
$dbidpermiso = $row["idpermiso"];
$dbdisplay = $row["display"];

echo '<p><a href="index.php?module=cfpcmods&section=permisos&action=del&id='.$fidpermiso.'" class="btnenlacecancel">Eliminar</a></p>';

?>
<form name="form1" action="index.php?module=cfpcmods&section=permisos&action=save" method="post">
<input type="hidden" name="haccion" value="update">
<?php echo '<input type="hidden" name="hidpermiso" value="'.$fidpermiso.'">'; ?>
...
<div class="row">
  ...
  <select name="lstmodulo" style="width: 100%;"/>
	...
	{
		if($dbidmod == $modulo["id"]){$seleccionado = ' selected=""';}else{$seleccionado = '';}
		echo '<option value="'.$modulo["idmod"].'" '.$seleccionado.'>'.$modulo["display"].'</option>';
	}
	?>
	</select>
  </div>
...
<div class="row">
  ...
  <div class="col" align="left">
    <?php echo '<input type="number" value="'.$dbidpermiso.'" name="txtidpermiso" required="" style="width: 100%;">'; ?>
  </div>
</div>
