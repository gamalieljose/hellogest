...
while($columna = mysqli_fetch_array($cnsprov))
{
    ...
    if ($columna["dft"] == '1')
{
    echo '<label><b><input name="chkfc[]" type="checkbox" value="'.$columna["id"].'" checked=""> '.$columna["serie"].'</b></label> ';
}
else
{
	echo '<label><input name="chkfc[]" type="checkbox" value="'.$columna["id"].'" > '.$columna["serie"].'</label> ';
}
