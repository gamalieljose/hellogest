...
while($columna = mysqli_fetch_array($cnsprov))
{
    ...
    if ($columna["dft"] == '1')
{
    echo '<label><b><input name="chkfvr[]" type="checkbox" value="'.$columna["id"].'" checked=""> '.$columna["serie"].'</b></label></br>';
}
else
{
	echo '<label><input name="chkfvr[]" type="checkbox" value="'.$columna["id"].'" > '.$columna["serie"].'</label></br>';
}
