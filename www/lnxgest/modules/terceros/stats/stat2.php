...
$ary_estadistica = array("Sector, 0");
...
$sqlcontactos= $mysqli->query("SELECT * FROM `".$prefixsql."dic_actividades` order by actividad");
	...
	echo '</tr>';
	 ...
	 $strestadistica = $col["actividad"].", ".$dbtodos;
	 //$strestadistica = $col["actividad"];
	 array_push($ary_estadistica,$strestadistica);
	}
?>
	</table>
...
<script type="text/javascript">
  ...
  function drawChart() {
	...
	['Actividad', 'Cantidad'],
	<?php
	$longitud = count($ary_estadistica);
	
	for($i=0; $i<$longitud; $i++)
      {
      //saco el valor de cada elemento
	  //echo $ary_estadistica[$i];
	  
	  $porciones = explode(",", $ary_estadistica[$i]);
	...
	var options = {
	  title: 'Terceros por Sector Actividad'
	};
