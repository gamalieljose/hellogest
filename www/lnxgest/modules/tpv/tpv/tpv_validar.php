...
$cnsaux = $mysqli->query("SELECT sum(importe) as sumabase from ".$prefixsql."tpv_lineas where idtpv = '".$idtpv."'");
$rowaux = mysqli_fetch_assoc($cnsaux);
$dbsumabase = $rowaux["sumabase"];

$cnsaux = $mysqli->query("SELECT sum(importe) as sumatax from ".$prefixsql."tpv_lineas_tax where idtpv = '".$idtpv."'");
$rowaux = mysqli_fetch_assoc($cnsaux);
$dbsumataxlineas = $rowaux["sumatax"];


$sumatotal = $dbsumabase + $dbsumataxlineas;

$sumatotal = round($sumatotal, 2);

// Calculamos si hay algo pagado, por ejemplo que se haya vuelto a borrador o simplemente se esté modificando
...
$rowaux = mysqli_fetch_assoc($cnsaux);
$dbsumapagos = $rowaux["sumapagos"];

$pendientedepago = $sumatotal - $dbsumapagos;
$pendientedepago = round($pendientedepago, 2);
?>
...
<div class="row">
    ...
    <div class="col" align="left">
        <?php echo '<b>'.$sumatotal.' Euros</b> Impuestos incluidos'; ?>
    </div>
</div>
<div class="row">
    <div class="col-sm-2" align="left">
    ...
    <div class="col" align="left">
<?php
if($pendientedepago > '0'){$permiteeditar = '';}else{$permiteeditar = ' readonly=""';}
        echo '<input type="text" name="txtpagado" '.$permiteeditar.' value="'.$pendientedepago.'" />';
?>
    </div>
</div>
